...
# Rows are front-left, front-right, back-left, and back-right. Columns are
# surge, sway, and yaw. This preserves the boat's existing thruster-test mixer.
THRUSTER_MIXER = (
    (1.0, 1.0, 0.5),
    (1.0, -1.0, -0.5),
    (-1.0, -1.0, 0.5),
...
def mix_thrusters(surge, sway, yaw):
    """Return normalized, desaturated outputs in FL, FR, BL, BR order."""
    commands = [
        row[0] * surge + row[1] * sway + row[2] * yaw
        for row in THRUSTER_MIXER
    ]
...
class ThrusterSerialNode(Node):
    ...
    def send_pwm(self, values):
        """Send one newline-delimited FL,FR,BL,BR command."""
    ...
    def watchdog_callback(self):
        ...
        if (not self.timed_out and
            ...
            self.send_pwm([NEUTRAL_PWM] * 4)
            self.timed_out = True
            self.get_logger().warning('cmd_vel timed out; thrusters set to neutral')
